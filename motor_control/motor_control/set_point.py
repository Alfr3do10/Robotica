...
class SetPointPublisher(Node):
    ...
    def timer_cb(self):
        ...
        # Generate signal based on the type
        if signal_type == 'sine':
            ...
            # For 'step' signal, alternate between positive and negative amplitude
            # Step signal is high (amplitude) for half the period, low (-amplitude) for the other half
            period = 1.0 / frequency
            if (elapsed_time % period) < (period / 2):
                self.signal_msg.data = amplitude  # Positive half
            else:
                self.signal_msg.data = -amplitude  # Negative half
        else:
